...
public:
     ...
     * @param *dst - output buffer for ecc     (ecc_length size at least) */
     void EncodeBlock(const void* src, void* dst) {
        ...
        // FIX: The polynomials for messages (MSG_CNT) are initialized with (msg_length + ecc_length) in constructor.
        // We MUST allocate enough stack space to cover that full size, otherwise the higher polynomial offsets
        // calculated in the constructor will point beyond the end of this stack array.
