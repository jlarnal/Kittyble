...
#define FAST_CPU         (ARCH_ESPRESSIF || defined(ARDUINO_ARCH_SAM) || defined(ARDUINO_ARCH_SAMD) || defined(ARDUINO_ARCH_STM32) || defined(TEENSYDUINO))
...
// the sequence between `noInterrupts()` and `interrupts()` calls.
    ...
    interrupts();
        ...
        value = (static_cast<unsigned long>(filler) << 24 | static_cast<unsigned long>(data[2]) << 16 | static_cast<unsigned long>(data[1]) << 8
    ...
    bool HX711::wait_ready_timeout(unsigned long timeout, unsigned long delay_ms, uint8_t retries)
    {
        ...
        delay_ms = pdMS_TO_TICKS(delay_ms);
        if (delay_ms < 1)
            delay_ms = 1;
